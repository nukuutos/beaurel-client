...
const useScroll = (form, setData) => {
  ...
  const lastMasterCardRef = useCallback(
    (node) => {
      ...
      observer.current = new IntersectionObserver(async (entries) => {
        ...
        if (element.isIntersecting && hasMore.current) {
          ...
          if (data?.cities.length && !isCancelled.current) {
            setData((cities) => [...cities, ...data.cities]);
          } else {
    ...
    [asyncAction, form, isCancelled, isLoading, setData]
  );
