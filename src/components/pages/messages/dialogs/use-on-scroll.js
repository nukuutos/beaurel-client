...
import { getDialogsOnScroll } from '../../../../redux/messages/actions';

const minFavoritesToLoad = 20;

const useOnScroll = () => {
  const [{ id: profileId, accessToken }, { dialogs }] = useSelector((state) => [
    state.auth,
    state.messages,
  ]);
  ...
  const isMinimalDataToLoad = dialogs.length >= minFavoritesToLoad;
    ...
    (node) => {
      ...
      observer.current = new IntersectionObserver(async (entries) => {
        ...
        if (element.isIntersecting && hasMore.current) {
          ...
          if (data?.dialogs.length) dispatch(getDialogsOnScroll(data));
